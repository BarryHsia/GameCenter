...
| memory-leak-analyzer | Android 内存泄漏静态分析 | 对话中提到内存泄漏分析时自动激活 |
...
扫描 Kotlin/Java 源码，检测常见 Android 内存泄漏模式（静态持有 Context、Handler 泄漏、未注销监听器、GlobalScope 等）。

使用示例：
```
分析项目中的内存泄漏风险
扫描 HomeActivity.kt 是否有内存泄漏
```

位置：`.kiro/skills/memory-leak-analyzer/`
